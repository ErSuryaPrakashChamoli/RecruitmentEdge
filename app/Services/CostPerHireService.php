 ...
 * optionally scoped to one requisition or department.
 */
class CostPerHireService
{
    public function totalCost(CarbonInterface $start, CarbonInterface $end, ?int $requisitionId = null, ?int $departmentId = null): float
    {
    ...
    public function successfulJoins(CarbonInterface $start, CarbonInterface $end, ?int $requisitionId = null, ?int $departmentId = null): int
    {
     ...
     */
    public function costPerHire(CarbonInterface $start, CarbonInterface $end, ?int $requisitionId = null, ?int $departmentId = null): ?float
    {
        $joins = $this->successfulJoins($start, $end, $requisitionId, $departmentId);

        if ($joins === 0) {
            return null;
        }

        return round($this->totalCost($start, $end, $requisitionId, $departmentId) / $joins, 2);
    }
}
