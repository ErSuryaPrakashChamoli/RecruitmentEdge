{{--
    A byte-for-byte copy of vendor/filament/widgets/resources/views/chart-widget.blade.php, with a
    single change: `:collapsed="true"` on the section. Needed because ChartWidget's own base view
    only reads $this->isCollapsible() — it never passes a default-collapsed state through to
    <x-filament::section>, so every other ChartWidget-based dashboard widget stays stuck open on
    first load regardless of the collapsible flag. Every other Dashboard section (custom Blade
    views) sets `collapsed` directly and needs no fork; this is the only way to get the same
    behavior for a ChartWidget short of patching the vendor package. Used by both
    TurnUpTrendChart and SourcePerformanceWidget via their own $view override — if Filament changes
    chart-widget.blade.php in a future upgrade, re-diff this file against the new vendor version.
--}}
@php
